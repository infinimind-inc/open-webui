...
export const getVideoPointerConfig = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/video_pointer`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const updateVideoPointerConfig = async (
	token: string,
	config: { VIDEO_POINTER_ALLOWED_PATHS: string[] }
) => {
	let error = null;
